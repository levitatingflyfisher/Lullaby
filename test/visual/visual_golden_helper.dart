...
Future<void> goldenAtSizes(
  ...
  for (final MapEntry<String, Size> size in sizes.entries) {
    for (final double scale in textScales) {
      ...
      // Capture the outermost MaterialApp (this helper's own). Use `.first`
      // rather than the bare type finder so a screen that supplies its own
      // nested `MaterialApp.router` (needed for GoRouter-dependent screens)
      // doesn't make the finder match two apps and fail.
      await expectLater(
